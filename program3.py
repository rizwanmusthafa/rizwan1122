...
def a_star(graph,heuristic,start,goal):
    ...
    while open_set:
        ...
        for neighor, cost in graph[current]:
            new_score = g_score[current]+cost
            if neighor not in g_score or new_score < g_score[neighor]:
                g_score[neighor]=new_score
                heapq.heappush(open_set,(new_score + heuristic[neighor],neighor))
                came_from[neighor]= current
    return None
